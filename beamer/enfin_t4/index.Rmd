---
title: "Opciones, derivados y futuros"
...
g1 <- ggplot(payoff.longcall) + geom_line(aes(x = periodo, y = payoff), 
             colour="red", size=1.5) + xlab("Tiempo de Expiracion") + 
             ylab("Payoff")
...
Para corroborar el gráfico ver la figura 9.1 (capítulo 9) del libro Introducción a los Mercados de Futuros y Opciones (octava edición) [@hull2003options].
...
g4 <- g4 + scale_x_continuous(limits=c(40, 130)) + 
...
g5 <- g5 + scale_x_continuous(limits=c(70, 130)) + 
...
g6 <- g6 + scale_x_continuous(limits=c(70, 130)) + 
           scale_y_continuous(limits=c(-30, 5))
g6
```
\vspace{8pt}
...
g8 <- g8 + scale_x_continuous(limits=c(40, 100)) + 
           scale_y_continuous(limits=c(-30, 7))
g8
```
Para efecto de académicos restringimos la escala del eje x e y (dimensión).
...
```r
g1 <- ggplot(bull.spread[4:10,]) + geom_line(aes(long.call$stock[4:10],
                                                 long.call$profit[4:10]),
                                                 linetype="dashed", 
                                                 color = "blue",size=1) 
g1 <- g1 + geom_line(aes(short.call$stock[4:10],short.call$profit[4:10]),
                     linetype="dashed", color = "red",size=1) 
g1 <- g1 + geom_line(aes(bull.spread$stock[4:10],bull.spread$profit[4:10]),
                     color = "green",size=1)
...
Imaginemos que es 31 de mayo del 2019 y queremos valorar las opciones call y put de [Oracle](https://www.oracle.com/index.html) con vencimiento al 20 de Septiembre del 2019. Para comenzar usamos la función `getOptionChain()` de la librería `quantmod`:  
...
Como debemos obtener el precio de cierre para el 31 de mayo del 2019 y a su vez el histórico de tres años para la volatilidad usamos:  
...
```{r,eval=TRUE,echo=FALSE,fig.align='center',fig.height=3.25,fig.width=6,results='hide'}
BinomialTreePlot(CRRTree, dy = 1, cex = 0.8, ylim = c(-6, 7), xlab = "n", 
                 ylab = "valores Opción")
title(main = "Arbol Binomial Opción")
```
...
```{r, eval=TRUE, echo=TRUE, fig.align='center', fig.height=3.2, fig.width=6, results='hide'}
# Americana
...
```r
BinomialTreePlot(CRRTree.1, dy = 1, cex = 0.8, ylim = c(-6, 7), xlab = "n", 
                 ylab = "valores Opción") 
...
```{r, eval=TRUE, echo=FALSE, fig.align='center', fig.height=3.2, fig.width=6, results='hide'}
...
1. [\faYoutubePlay](https://youtu.be/b54LK6TRjws) Black & Sholes 

2. [\faYoutubePlay](https://youtu.be/Qz8pdhf7yyU) Black & Sholes: Con datos reales

3. [\faYoutubePlay](https://youtu.be/2LCceV1-4Yg) Árbol Binomial 
