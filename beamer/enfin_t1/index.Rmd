...
3. Otra opción es simplemente cargar las funciones, sin necesidad de instalar y cargar la librería. 
...
## Ratio de Sharpe 
Este ratio representa una medida del exceso de rendimiento por unidad de riesgo de una inversión.
$$
\begin{aligned}
Sharpe= \frac{\mu_{p,t}-r_f}{\sigma_{p,t}}   \\
\end{aligned}
$$
[@sharpe1994sharpe]

## Alpha de Jensen
[@jensen1968performance]
Representa el exceso de retorno anormal de un portafolio, de acuerdo a su valor teórico esperado.
Podemos representar esta medida como:
$$
\begin{aligned}
\alpha _{J}=R_{i}-[R_{f}+\beta _{{iM}}\cdot (R_{M}-R_{f})]
\end{aligned}
$$
Donde $\alpha _{J}$ representa el coeficiente de posición y $\beta _{{iM}}$ el estimador de la siguiente regresión 
$$
\begin{aligned}
R_{i}-R_{f}= \alpha _{J}+\beta _{{iM}}\cdot (R_{M}-R_{f})
\end{aligned}
$$
...
stocks1 <- xts(x = stocks, order.by = stocks$Date)

```

```{r, eval = TRUE, echo = TRUE}
Ra <- stocks1$Nordstrom
Rb <- stocks1$mercado

CAPM.jensenAlpha(Ra, Rb, Rf = 0.005)
```
```{r, eval = TRUE, echo = TRUE}
modelo=lm(stocks$Nordstrom~stocks$mercado)
modelo[1]
```
## Medida de Treynor
$$
...
$$
Donde $r_{i}$ es el rendimiento del portafolio $i$, $r_{f}$ la tasa libre de riesgo y $\beta_{i}$ el beta de la cartera.

```{r, eval = TRUE, echo = TRUE}
print(TreynorRatio(stocks$Nordstrom, stocks$mercado, Rf = 0.005, scale = NA, modified = FALSE))
```
